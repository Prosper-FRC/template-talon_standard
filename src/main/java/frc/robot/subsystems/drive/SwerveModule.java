...
public class SwerveModule {
    ...
    public void setSpeed(SwerveModuleState desiredState, boolean isOpenLoop) {
        if (isOpenLoop) {
            driveMotor.setControl(driveVoltageControl.withOutput(12 * (desiredState.speedMetersPerSecond / kMaxSpeed)));  // Open-loop speed control as percent output
        } else {
            driveMotor.setControl(
                velocityControl.withVelocity(desiredState.speedMetersPerSecond)
                .withFeedForward(feedforward.calculate(desiredState.speedMetersPerSecond)));  // Closed-loop velocity control
        }
    }
    ...
    public void resetToAbsolute() {
        azimuthMotor.setPosition(Rotation2d.fromRotations(absolutePositionSignal.getValueAsDouble()).minus(angleOffset).getRotations(), 2.5);  // Set motor to absolute position
    }
